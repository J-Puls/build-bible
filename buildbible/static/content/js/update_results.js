            ...
            if (json.length > 0) {
                if (category > 0) {
                    ...
                    for (var i = 0; i < json.length; i++) {
                        $('.posts').append(`<section class="post">
                        <header class="post-header">
                            <h2 class="post-title">` + json[i].display_filename + `</h2>
                            <p class="post-meta">
                                Uploaded on: <small>` + new Date(json[i].date_uploaded).toLocaleDateString("en-US", options) + `</small>
                            </p>
                        </header>
                            <a href="/media/` + json[i].fsm_file + `" target="_blank" class="pure-button button-link" aria-label="View PDF" title="View PDF">View PDF</a>
                    </section>`)
                    }
                }
